...
class GANet(SingleStageDetector):
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        if self.head:
            [kpts_hm, pts_offset, int_offset] = self.bbox_head.forward_train(output['features'],
    ...
    def test_inference(self, img, hack_seeds=None, **kwargs):
        ...
        if self.head:
            [kpts_hm, pts_offset, int_offset] = self.bbox_head.forward_train(output['features'],
                                                                                      output.get("aux_feat", None))
            [cpt_seeds, kpt_seeds] = self.bbox_head.forward_test(output['features'], output.get("aux_feat", None), hack_seeds,
        ...
        output['seeds'] = [cpt_seeds, kpt_seeds]
        output['hm'] = None
        return output
