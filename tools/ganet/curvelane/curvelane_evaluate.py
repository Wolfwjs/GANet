...
def spline_interp(*, lane, step_t=1):
    ...
    if len(lane) < 2:
        return lane
    interp_param = calc_params(lane) # # list[dict_keys(['a_x', 'b_x', 'c_x', 'd_x', 'a_y', 'b_y', 'c_y', 'd_y', 'h'])]
...
def calc_params(lane):
    ...
    for i in range(n_pt - 2):
        ...
        dy2 = (lane[i + 2]['y'] - lane[i + 1]['y']) / h[i + 1] ## h[i+1]==0
        tmpy = 6 * (dy2 - dy1)
        if i == 0:
            C[i] /= B[i]
            D_x.append(tmpx / B[i])
...
def calc_iou(lane1, lane2, hyperp): # {'eval_width': 224, 'eval_height': 224, 'iou_thresh': 0.5, 'lane_width': 5}
    ...
    lane_width = hyperp['lane_width'] # 5

    im1 = np.zeros((new_height, new_width), np.uint8)
    im2 = np.zeros((new_height, new_width), np.uint8)
    interp_lane1 = spline_interp(lane=lane1, step_t=1) # xy_dict点集合
    interp_lane2 = spline_interp(lane=lane2, step_t=1)

    for i in range(0, len(interp_lane1) - 1): # 这样计算iou还是比较方便的
    ...
    union_im = cv2.bitwise_or(im1, im2) # 取or
...
def evaluate_core(*, gt_lanes, pr_lanes, gt_wh, pr_wh, hyperp):
    ...
    if gt_num > 0 and pr_num > 0:
        ...
        gt_y_ratio = np.true_divide(gt_wh['height'], new_height) # 1440,224
        gt_x_ratio = np.true_divide(gt_wh['width'], new_width) # 2560,224
        ...
        sorted_gt_lanes = list(map(lambda lane: resize_lane(lane, gt_x_ratio, gt_y_ratio), gt_lanes)) 
        sorted_pr_lanes = list(map(lambda lane: resize_lane(lane, pr_x_ratio, pr_y_ratio), pr_lanes)) # 都变成了224尺度的

        iou_mat = np.zeros((gt_num, pr_num)) # (4, 3)

        for (index_gt, gt_lane), (index_pr, pr_lane) in product(enumerate(sorted_gt_lanes), enumerate(sorted_pr_lanes)): # 每条gt和每条pred之间迭代
        ...
        cost_matrix = 1 - np.array(iou_mat) # iou越小cost越大
        match_index_list = linear_sum_assignment(cost_matrix) # (array([0, 1, 2]), array([1, 2, 0])),也就是4个gt和3个pred之间的对应

        for gt_index, pr_index in zip(*match_index_list):
            iou_val = iou_mat[gt_index][pr_index]
            if iou_val > iou_thresh: # 0.5
                hit_num += 1
...
class LaneMetricCore(object):
    ...
    def __call__(self, gt_result, pr_result, *args, **kwargs):
        ...
        :return:
        """
        prob_thresh = self.prob_thresh # none
        ...
        gt_lanes = []
        for line_spec in target_spec['Lines']: # 有3个lane
            if len(line_spec) > 0:
        ...
        result = evaluate_core(gt_lanes=gt_lanes, pr_lanes=pr_lanes, gt_wh=gt_wh, pr_wh=pr_wh, hyperp=self.eval_params) # {'gt_num': 4, 'pr_num': 3, 'hit_num': 0, 'pr_list': [False, False, False], 'gt_list': [False, False, False, False]}
    ...
    def summary(self):
        ...
        hit_num = sum(result['hit_num'] for result in self.result_record) # len=1,100,200
        ...
        return dict(F1=F1, precision=precision, recall=recall, pr_num=pr_num, gt_num=gt_num)
