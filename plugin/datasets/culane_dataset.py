...
class CulaneDataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        path = self.img_infos[idx]
        imgname = os.path.join(self.img_prefix, path if path[0]!='/' else path[1:])
    ...
    def load_labels(self, idx, offset_x, offset_y):
        ...
        anno_dir = os.path.join(self.img_prefix, path if path[0]!='/' else path[1:])
        shapes = []
